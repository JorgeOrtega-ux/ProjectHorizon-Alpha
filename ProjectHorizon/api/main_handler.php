...
// --- FUNCIÓN DE SEGURIDAD CORREGIDA Y MEJORADA ---
function handle_security_event($conn, $identifier, $action, &$custom_message = '') {
    ...
    if ($action === 'log_attempt' || $action === 'log_reset_fail') {
        ...
        $stmt = $conn->prepare("INSERT INTO security_logs (user_identifier, action_type) VALUES (?, ?)");
        $stmt->bind_param("ss", $identifier, $log_action_type);
        $stmt->execute();
        $stmt->close();
        return;
    }
    ...
    if ($action === 'check_reset_request') {
        ...
        $stmt_log = $conn->prepare("INSERT INTO security_logs (user_identifier, action_type) VALUES (?, 'reset_request')");
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    // ... (El resto del código GET no necesita cambios, se mantiene igual)
...
elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action_type === 'login_user') {
        ...
        if ($user = $result->fetch_assoc()) {
            ...
            handle_security_event($conn, $email, 'log_attempt');
            $lock_message = '';
            // ✅ **MEJORA DE CONSISTENCIA**: Comprobar bloqueo también si el usuario no existe.
            if (handle_security_event($conn, $email, 'check_lock', $lock_message)) {
                http_response_code(429);
                echo json_encode(['success' => false, 'message' => $lock_message]);
            } else {
